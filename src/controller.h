...
class Controller {
  ...
  */
  ...
  /**
   * @brief Calls arena method to handle adding/removing lights
   *
   * @param num the number passed by the GUI to change lights to
   */
  void ChangeNumLights(int num) { arena_->ChangeNumLights(num); }

  /**
   * @brief Calls arena method to handle changing light sensor numerator
   *
   * @param num the new numerator value
   */
  void setSensorReadings(int num) { arena_->setLightReading(num); }

  /**
   * @brief Calls arena method to handle adding/removing food
   *
   * @param num the number passed by the GUI to change food to
   */
  void ChangeNumFood(int num) { arena_->ChangeNumFood(num); }

  /**
   * @brief Calls arena method to handle chaning robot food_flag_
   *
   * @param[in] flag bool value to change robot's food_flag_ to
   */
  void FlipRobotHunger(bool flag) { arena_->FlipRobotHunger(flag); }

  /**
   * @brief Calls arena method to handle adding/removing robots
   *
